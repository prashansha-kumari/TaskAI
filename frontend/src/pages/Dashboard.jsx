...
function Dashboard({ setPage }) {
  ...
  const fetchTasks = async () => {
    const res = await axios.get(
      "https://taskai-backend-lsyq.onrender.com/api/tasks",
      {
  ...
  const toggleComplete = async (task) => {
    await axios.put(
      `https://taskai-backend-lsyq.onrender.com/api/tasks/${task._id}`,
  ...
  const deleteTask = async (id) => {
    await axios.delete(
      `https://taskai-backend-lsyq.onrender.com/api/tasks/${id}`,
      {
        headers: { Authorization: `Bearer ${token}` },
      },
    );
  ...
  const fetchNotes = async () => {
    const res = await axios.get(
      "https://taskai-backend-lsyq.onrender.com/api/notes",
      {
        headers: { Authorization: `Bearer ${token}` },
      },
    );
  ...
  const deleteNote = async (id) => {
    await axios.delete(
      `https://taskai-backend-lsyq.onrender.com/api/notes/${id}`,
      {
        headers: { Authorization: `Bearer ${token}` },
      },
    );
  ...
  const askAI = async () => {
    ...
    const res = await axios.post(
      "https://taskai-backend-lsyq.onrender.com/api/ai/ask",
      { question },
      { headers: { Authorization: `Bearer ${token}` } },
    );
  ...
  const fetchStats = async () => {
    const res = await axios.get(
      "https://taskai-backend-lsyq.onrender.com/api/tasks/stats",
      {
        headers: { Authorization: `Bearer ${token}` },
      },
    );
  ...
  bg-gradient-to-br from-[#0f0c29] via-[#302b63] to-[#24243e] text-white"
    >
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-purple-300">TaskAI</h1>
        <button
    ...
    bg-white/5 backdrop-blur-lg border border-white/10"
      >
        ...
        <div className="flex flex-col gap-2">
          ...
          {answer && (
            <div className="p-3 rounded bg-white/10 mt-2">{answer}</div>
          )}
        </div>
      </div>
    </div>
